...
Команда `analyze-replacements` анализирует лог распознавания через Anthropic Agent SDK и предлагает новые правила автозамены. Каждая распознанная строка подаётся модели вместе с контекстом (±N соседних распознанных строк), модель находит систематические ошибки ASR для технических терминов.
...
# За конкретный период (игнорирует чекпоинт)
...
| Флаг | Env | Назначение |
|------|-----|-----------|
| `--context N` | `CONTEXT_LINES` | Строк контекста ±N вокруг каждой строки |
| `--batch-size N` | `BATCH_SIZE` | Окон в одном запросе к модели |
| `--max-iterations N` | `MAX_ITERATIONS` | Лимит запросов к модели за прогон (0 = без лимита) |
| `--min-freq N` | `MIN_FREQ` | Минимальная частота строки для анализа |
| `--min-confidence` | `MIN_CONFIDENCE` | Отсекать кандидатов ниже уровня (low/medium/high) |
| `--model` | `MODEL` | Модель Anthropic (по умолчанию claude-opus-4-8) |
