...
# Метод ввода текста
SHEPTUN_USE_CLIPBOARD=true        # true — через буфер обмена, false — через CGEvent
```
...
### CGEvent (альтернативный)

Текст вводится напрямую через macOS Quartz Events (`CGEventKeyboardSetUnicodeString`).

**Ограничения:**
- Дублирование текста в терминале VS Code
- Некоторые приложения обрабатывают события по-разному
