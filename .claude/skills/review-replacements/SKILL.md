...
### Шаг 3.5. Найти конфликты МЕЖДУ правилами

Проверка по одному правилу такие дефекты не ловит: каждое по отдельности разумно, ломает
их сочетание. Прогони детерминированный скан — он работает по всему файлу независимо от
области ревью:

```bash
python3 .claude/skills/review-replacements/scripts/find_conflicts.py
```

Что он ищет и что с этим делать:

- **Обратные пары** (A→B и B→A) — словарь одновременно объявляет каноном обе формы,
  результат зависит от того, какое правило совпало первым. Оставить ОДНО направление.
  Реальный случай: `"commit" → "коммит"` (freq=420) и `"коммит" → "commit"` (freq=1771)
  породили разные батчи анализатора; пара стоила 3 п.п. CER и 4 п.п. EPI.
- **Порча терминов** — правило переводит в кириллицу слово, которое другие правила
  считают эталонной латиницей. Это же слово подсказывается декодеру
  (`gigaam.load_hotwords`), поэтому правило прямо гасит биасинг. Почти всегда — удалять.
- **Мёртвые цепочки** — значение правила является ключом другого. Замена однопроходная
  (`commands.py:apply_replacements`), вторая ступень НЕ сработает: `"комид" → "коммит"`,
  и дальше в `commit` слово уже не пойдёт. Не ошибка сама по себе, но автор правила мог
  ждать другого результата — свериться с намерением, при расхождении поправить значение.

Конфликты первых двух типов включай в отчёт наравне с вердиктами по отдельным правилам.
