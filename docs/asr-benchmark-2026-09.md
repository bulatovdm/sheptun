...
### Задержка и резка длинных фраз

Замер на простаивающем GPU (распознавание + постобработка, записи тест-сета и склейки по 4):

| Звук | greedy (утро) | LM + hotwords-652 | + резка фраз >10 с на паузах |
|---|---:|---:|---:|
| ~5 с (медиана живых фраз 4,9 с) | 36 мс | 79 мс | 64 мс |
| ~19 с (5% живых фраз длиннее 15 с) | 72 мс | 486 мс | 198 мс |

Почти всё время — KenLM в `pyctcdecode` (чистый Python): он дорожает быстрее длины звука, а
ширина луча (50→15) и число hotwords (652→200) почти не влияют. Резка кадров посередине пауз
(≥8 пустых кадров CTC, ~0,3 с) и склейка кусков: декодинг 314→89 мс, CER на длинных склейках
5,4%→6,0% (на стыке LM теряет контекст) — поэтому только для фраз длиннее
`SHEPTUN_GIGAAM_SPLIT_SECONDS` (10 с, 14% живых фраз). Сеть и очистка — ~3 мс.
Опробовано и хуже: `beam_prune_logp=-5` (210 мс, расхождение 3%), `token_min_logp=-3` (325 мс).
